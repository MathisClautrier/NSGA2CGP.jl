...
function get_outputs(ind::NSGA2CGPInd)::Array{Float64}
...
function set_inputs(ind::NSGA2CGPInd, inputs::Array{Float64})::Nothing
    for i in eachindex(inputs)
        ind.buffer[i] = inputs[i]
    end
end

function process(ind::NSGA2CGPInd)::Array{Float64}
...
function process(ind::NSGA2CGPInd, inputs::Array{Float64})::Array{Float64}
